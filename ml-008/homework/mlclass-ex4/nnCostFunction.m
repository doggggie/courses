...
for i = 1:m
    ...
    % yvec(i,:) 1x10
    J = J - yvec(i,:) * log(h) - (ones(size(yvec(i,:))) - yvec(i,:)) * log(ones(size(h)) - h);
    

    % Back prop - compute grad's
    delta3 = h - yvec(i,:)'; % 10x1
    gp = sigmoidGradient(z2); % 25x1
    gp = [0; gp]; % 26x1
    delta2 = (Theta2' * delta3) .* gp; % 26x1
...
temp1 = Theta1;
temp1(:,1) = 0;
J = J + 0.5 * lambda * sum(sum(temp1 .* temp1)) / m;

temp2 = Theta2;
temp2(:,1) = 0;
J = J + 0.5 * lambda * sum(sum(temp2 .* temp2)) / m;
